...
class A5P7
{
  ...
  private
  boolean doesNDivide( int a, int b,int n ) {
  
    return ( ( a+(n-1)*b ) == 0 ) || ( Math.abs( a+(n-1)*b )/n >= 1 && Math.abs( a+(n-1)*b )%n == 0 );
  }
  
  private
  boolean doesNMDivide( int a, int b, int n,int m ) {
    
    return ( ( m*a+( n-m )*b ) == 0 ) || (
      Math.abs( m*a+( n-m )*b )/n >= 1 && Math.abs( m*a+( n-m )*b )%n == 0 );
  }
  ...
  A5P7( int aLo, int aHi, int bLo, int bHi, int n, int m ) {
    ...
    sbSpaced.append(
      "\n"+ConsolColor.cyan_brt+"denotes where "+n+"|("+m+"*a +"+(n-m)+"*b) and a < b"+ConsolColor.RESET+
      "\n"+ConsolColor.rd_brt+"denotes where "+n+"|("+m+"*a +"+(n-m)+"*b)"+ConsolColor.RESET+
      "\n"+ConsolColor.grn_brt+"denotes where "+n+"|("+m+"*a +"+( n-m )+"*b)\n\n"+ConsolColor.RESET )
            .append( aAxisBounds ).append( bAxisBounds )
            .append( "Figure 7.1: Stretched version of graph showing (a,b) values on the axis."+
                     "\nWhere a is the horizontal-axis, and b is the vertical-axis.\n" );
    ...
    for( int b = bHi; b >= bLo; --b ) {
      ...
      for( int a = aLo; a<=aHi; ++a ) {
        ...
        boolean a2b = doesNMDivide( b, a, n,m );
